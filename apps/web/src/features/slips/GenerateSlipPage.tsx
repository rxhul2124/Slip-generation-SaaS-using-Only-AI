...
export function GenerateSlipPage() {
  ...
  const customerProducts = useMemo(() => selectedCustomer?.products || [], [selectedCustomer]);
  ...
  useEffect(() => {
    if (productId && !customerProducts.some((product) => product._id === productId)) setProductId("");
  ...
  const draftSlip = useMemo<GeneratedSlip>(() => {
    return {
      ...
      orderReference,
      destination: selectedCustomer?.shippingAddress?.city || "",
      notes,
      signature: {
  ...
  const createSlip = useMutation({
    mutationFn: async () => {
      ...
      return resources.slips.create({
        ...
        orderReference,
        notes,
        destination: draftSlip.destination,
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid gap-3 lg:grid-cols-2">
              <div className="rounded-md border bg-muted/35 p-3 text-sm">
                ...
                <div className="flex flex-wrap gap-2">
                  {slipTypeDefinition.visibleFields.map((field) => (
